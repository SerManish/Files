<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    <xsl:import href="https://payment-files.mykaarma.com/payment-report-xslt/payment_report_total.xslt" />
    <xsl:template match="/">
        <html>
            <head>
            ...
            <body>
                <br />
                <br />
                <b>Daily Close [New Format]</b>
                <br />
                <b>
                    <u>
                        Dealer:
                        <xsl:value-of select="PaymentReport/DealerName" />
                ...
                Close Date:
                <b>
                    <xsl:value-of select="PaymentReport/CloseDate" />
                    <xsl:text disable-output-escaping="yes">&nbsp;</xsl:text>
                </b>
                <br />
                Daily Transactions from
                <b>
                    <xsl:value-of select="PaymentReport/CloseStartTime" />
                    <xsl:text disable-output-escaping="yes">&nbsp;</xsl:text>
                </b>
                to
                <b>
                    <xsl:value-of select="PaymentReport/CloseEndTime" />
                    <xsl:text disable-output-escaping="yes">&nbsp;</xsl:text>
                </b>
                <br />
                ...
                Transaction Count:
                <b>
                    <xsl:value-of select="PaymentReport/TransCount" />
                    <xsl:text disable-output-escaping="yes">&nbsp;</xsl:text>
                </b>
                <br />
                <br />
                <xsl:for-each select="PaymentReport/Departments/Department">
                    <xsl:if test="current()/Row">
                        ...
                        <xsl:call-template name="transaction_table" />
    ...
    <xsl:template name="transaction_table">
        <br />
        <table width="1100px" style="border-width: 1px; border-style: solid; border-color: gray; border-collapse: collapse; background-color:white;">
            ...
            <xsl:choose>
                <xsl:when test="$isEven = 1">
                    <xsl:if test="$rowtype='Transaction'">
                        <xsl:call-template name="transaction_table_body_even_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='CashierName'">
                        <xsl:call-template name="transaction_table_body_service_advisor_even_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='SubTotal'">
                        <xsl:call-template name="transaction_table_body_subtotal_even_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='RefundDescription'">
                        <xsl:call-template name="transaction_table_body_refunddescription_even_row" />
                    </xsl:if>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:if test="$rowtype='Transaction'">
                        <xsl:call-template name="transaction_table_body_odd_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='CashierName'">
                        <xsl:call-template name="transaction_table_body_service_advisor_odd_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='SubTotal'">
                        <xsl:call-template name="transaction_table_body_subtotal_odd_row" />
                    </xsl:if>
                    <xsl:if test="$rowtype='RefundDescription'">
                        <xsl:call-template name="transaction_table_body_refunddescription_odd_row" />
                    </xsl:if>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
    ...
    <xsl:template name="final_totals_table">
        <br />
        <table style="border-width: 0px; border-collapse: collapse; background-color:white;" width="1100px">
